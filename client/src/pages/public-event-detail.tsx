      ...
      <Card 
        className={`relative overflow-hidden transition-all duration-500 ${
          wasJustAdded 
            ...
            : "bg-gradient-to-br from-slate-900/80 to-slate-800/60 border-white/10 hover:border-yellow-500/30"
        }`}
          ...
          {wasJustAdded && (
            ...
            >
              <div className="text-center">
                ...
                <p className="text-lg font-semibold text-white">Aggiunto al carrello!</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <CardTitle className="text-xl text-white" data-testid={`text-sector-name-${sector.id}`}>
                  {sector.name}
                </CardTitle>
              </div>
            </div>
            <div className="text-right">
              ...
              {sector.priceRidotto && Number(sector.priceRidotto) > 0 && (
                <p className="text-xs text-slate-400">Ridotto: €{Number(sector.priceRidotto).toFixed(2)}</p>
              )}
            </div>
          </div>
        </CardHeader>
            ...
            <>
              {sector.priceRidotto && Number(sector.priceRidotto) > 0 && (
                <div className="space-y-2">
                  <Label className="text-sm text-slate-300 font-medium">Tipo Biglietto</Label>
                  <RadioGroup value={ticketType} onValueChange={setTicketType} className="flex gap-3">
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="intero" id={`intero-${sector.id}`} className="border-yellow-500" />
                        <Label htmlFor={`intero-${sector.id}`} className="text-white cursor-pointer font-medium">
                          Intero
                        </Label>
                      </div>
                      <p className="text-yellow-400 font-bold mt-1">€{Number(sector.priceIntero).toFixed(2)}</p>
                    </div>
                    <div 
                      className={`flex-1 p-3 rounded-xl border cursor-pointer transition-all ${
                        ticketType === "ridotto" 
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="ridotto" id={`ridotto-${sector.id}`} className="border-yellow-500" />
                        <Label htmlFor={`ridotto-${sector.id}`} className="text-white cursor-pointer font-medium">
                          Ridotto
                        </Label>
                      </div>
                      <p className="text-yellow-400 font-bold mt-1">€{Number(sector.priceRidotto).toFixed(2)}</p>
                    </div>
                  </RadioGroup>
                </div>
              )}
              ...
              {sector.isNumbered ? (
                <div className="space-y-2">
                  <Label className="text-sm text-slate-300 font-medium">Seleziona Posto</Label>
                  <div className="grid grid-cols-6 gap-2 max-h-40 overflow-y-auto p-3 bg-black/30 rounded-xl">
                ...
                <div className="space-y-2">
                  <Label className="text-sm text-slate-300 font-medium">Quantità</Label>
                  <div className="flex items-center gap-4 bg-black/30 rounded-xl p-2">
                    <Button
                      variant="ghost"
                      size="icon"
                    ...
                    <span className="text-2xl font-bold text-white w-12 text-center" data-testid={`text-quantity-${sector.id}`}>
                      {quantity}
                    </span>
                    <Button
                      variant="ghost"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="Mario"
                      className="bg-black/30 border-white/10 text-white placeholder:text-slate-500 rounded-xl"
                      data-testid={`input-firstname-${sector.id}`}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-sm text-slate-300 font-medium">Cognome</Label>
                    <Input
                      value={lastName}
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/10">
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wider">Totale</p>
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-black font-bold px-6 py-3 rounded-xl shadow-lg shadow-yellow-500/25 transition-all hover:shadow-xl hover:shadow-yellow-500/30 hover:scale-105"
      ...
      <div className="min-h-screen bg-[#0a0e17] flex items-center justify-center p-4">
        ...
        >
          <Card className="p-8 text-center bg-red-500/10 border-red-500/20 max-w-md backdrop-blur-xl">
            ...
            <h2 className="text-xl font-bold text-white mb-2">Evento non trovato</h2>
            <p className="text-slate-400 mb-6">L'evento richiesto non è disponibile o non esiste.</p>
            <Link href="/acquista">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#0a0e17]/80 border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-4">
            <Link href="/acquista">
              <Button variant="ghost" className="text-slate-300 hover:text-white hover:bg-white/10" data-testid="button-back">
            ...
            <Link href="/">
              <div className="flex items-center gap-2 cursor-pointer group">
                ...
                <span className="text-xl font-bold text-white hidden sm:block">Event4U</span>
              </div>
            </Link>
            <Link href="/carrello">
              <Button className="relative bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-black font-bold rounded-xl shadow-lg shadow-yellow-500/25" data-testid="button-cart">
        ...
        {isLoading ? (
          ...
          >
            <Card className="relative overflow-hidden rounded-3xl border-0 bg-gradient-to-br from-indigo-900/60 via-purple-900/50 to-pink-900/40">
              ...
              <CardContent className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 text-white/90">
            ...
            {event.eventDescription && (
              ...
              >
                <Card className="bg-slate-900/50 border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                  <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
                    <Star className="w-5 h-5 text-yellow-400" />
                    Descrizione
                  </h3>
                  <p className="text-slate-300 whitespace-pre-line leading-relaxed" data-testid="text-event-description">
                    {event.eventDescription}
                  </p>
                </Card>
              </motion.div>
            ...
            {event.sectors.length === 0 && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
              >
                <Card className="p-12 text-center bg-slate-900/50 border-white/10 rounded-2xl">
                  <Ticket className="w-16 h-16 mx-auto mb-4 text-slate-600" />
                  <h3 className="text-xl font-semibold text-white mb-2">Nessun biglietto disponibile</h3>
