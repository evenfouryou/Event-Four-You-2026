...
1. **C1 Reports (Registro Titoli di Accesso)** - Used for manual transmissions via `/api/siae/ticketed-events/:id/reports/c1/send`:
   - **UNICO GENERATORE**: `generateC1ReportXml()` in `server/siae-routes.ts`
   - **Giornaliero**: Root element `<RiepilogoGiornaliero Data="YYYYMMDD">`, file `RMG_YYYY_MM_DD_###.xsi`
   - **Mensile**: Root element `<RiepilogoMensile Mese="YYYYMM">`, file `RPM_YYYY_MM_###.xsi`
   - Structure: `RiepilogoGiornaliero/Mensile → Titolare → Organizzatore → Evento → OrdineDiPosto → TitoliAccesso + Abbonamenti`
   - **Mandatory elements**: `<Incidenza>`, `<ImponibileIntrattenimenti>` inside `<Intrattenimento>`
   ...
   - Signed files use `.xsi.p7m` extension per Allegato C SIAE

2. **RCA Reports (Riepilogo Controllo Accessi)** - Used for automated nightly transmissions via scheduler:
   - Generated automatically at 02:00 (daily) and 03:00 first of month (monthly)
   - Generated by `generateXMLContent()` in `server/siae-scheduler.ts`
   ...
   - File naming: `RCA_YYYY_MM_DD_###.xsi`

#### Shared SIAE Utilities (`server/siae-utils.ts`)
Centralized helper functions for SIAE compliance:
- `escapeXml()`: UTF-8 compliant XML character escaping
- `formatSiaeDateCompact()`: Date to YYYYMMDD format
- `formatSiaeTimeCompact()`: Time to HHMMSS format
- `formatSiaeTimeHHMM()`: Time to HHMM format (OraEvento)
- `generateSiaeFileName()`: Generates compliant filenames (RMG/RPM/RCA prefixes)
- `normalizeSiaeTipoTitolo()`: Normalizes ticket type codes (R1, R2, O1, ABB)
- `normalizeSiaeCodiceOrdine()`: Normalizes sector codes (A0 default)
